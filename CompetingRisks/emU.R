emU <- function(t, d, z, x, zetat, zetaz, theta = 0.5, iter = 20, weights = NULL){
  #t is a vector of n, d is matrix n*2, zetat is a vector of 2, zetaz is a scaler
  ...
  for(j in 1:iter){
    bh1 = basehaz(U.fit1, centered=F) #cumulative baseline hazard for remission with mean(offset)
    index1 = match(t,bh1$time)
    
    bh2 = basehaz(U.fit2, centered=F) #cumulative baseline hazard for surgery with mean(offset)
